...
const weatherImgDay = function (el, text) {
  if (text.includes(("rain" && "heavy") || ("torrential" && "rain"))) {
    ...
    text.includes("snow" || "blizzard" || ("freezing" && "drizzle") || "ice")
  ) {
    el.src = "./img/snow.png";
  } else if (
    text.includes(
      ("light" && "rain") || ("moderate" && "rain") || ("light" && "sleet")
    )
  ) {
...
const getCity = async function (lat, lng) {
  try {
    ...
    if (!geo.ok)
      throw new Error("We are getting problem of geolocalization you😢");
...
const geoLocal = function () {
  ...
  getPosition()
    ...
    .catch(err => {
      console.log(err);
      renderError(
        `Where are you?? 🤨  Try again! <br> Let us find you, Enable geolocation 😀 or type your city!`
      );
    });
};
